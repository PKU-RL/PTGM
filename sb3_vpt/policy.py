...
class VPTPolicy(RecurrentActorCriticPolicy):
    def __init__(
        ...
        **kwargs\
        ...
        finetune_full = kwargs.pop("finetune_full", False)
        ...
        if weights_path:
            # if not finetune_full, params for adapters cannot be loaded
            self.model.load_state_dict(th.load(weights_path), strict=finetune_full)
            self.exploration_model.load_state_dict(th.load(weights_path), strict=finetune_full)
