...
class PPOPolicy(RecurrentActorCriticPolicy):
    def __init__(
        self,
        ...
        **kwargs\
        ...
        policy_action_space = kwargs.pop("policy_action_space", None)
        ...
        if weights_path and not initialize_with_prior:
            # load part of the model params shared with VPT for initialization
            self.model.load_part_vpt_weights(th.load(weights_path))
        elif initialize_with_prior:
            # initialize the model with the goal prior
            self.model.load_weights(prior_path, strict=True)

        self.model.requires_grad_(False)
        self.params = {}

        '''
        # try: train the whole model
        self.model.net.requires_grad_(True)
        self.params["model.net"] = self.model.net.parameters()
        '''
        
        # Fix image encoder & transformer, optimize others
        if not initialize_with_prior:
        ...
        if not initialize_with_prior:
            self.model.pi_head.reset_parameters()
        self.model.pi_head.requires_grad_(True)
        self.params["model.pi_head"] = self.model.pi_head.parameters()

        self.model.net.lastlayer.requires_grad_(True)
        self.params["model.net.lastlayer"] = self.model.net.lastlayer.parameters()
        self.model.net.final_ln.requires_grad_(True)
        self.params["model.net.final_ln"] = self.model.net.final_ln.parameters()
        #self.model.net.recurrent_layer.requires_grad_(True)
        #self.params["model.net.recurrent_layer"] = self.model.net.recurrent_layer.parameters()
        ...
        print("Model setup. Params: {}. Optimize params: {}.".format(num_param, num_param_grad))
    ...
    def forward(self, 
        ...
        #ac = th.cat([x[:, 0] for x in ac.values()], dim=1)     
        #print(ac)         
        ac = ac[:,0,:] # batch x 512

        return ac, vpred, log_prob, self._vpt_states_to_sb3(state_out), pd
    ...
    def evaluate_actions(
        ...
        #}
        #print(actions.shape, pd.shape)
        log_prob = self.model.pi_head.logprob(actions.reshape((n_seq, max_len, -1)), pd)     # n_seq x max_len
